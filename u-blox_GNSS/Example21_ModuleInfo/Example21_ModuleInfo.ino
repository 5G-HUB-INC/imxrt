...
void setup()
{
  ...
  //      ; //Wait for user to open terminal
    ...
    DSerial.print(F("Polling module info"));
    if (myGNSS.getModuleInfo(1100) == false) // Try to get the module info
    {
        DSerial.print(F("getModuleInfo failed! Freezing..."));
        while (1)
            ;
    }
    ...
    DSerial.println();
    DSerial.println();
    DSerial.println(F("Module Info : "));
    DSerial.print(F("Soft version: "));
    DSerial.println(myGNSS.minfo.swVersion);
    DSerial.print(F("Hard version: "));
    DSerial.println(myGNSS.minfo.hwVersion);
    DSerial.print(F("Extensions:"));
    DSerial.println(myGNSS.minfo.extensionNo);
    for (int i = 0; i < myGNSS.minfo.extensionNo; i++)
    {
        DSerial.print("  ");
        DSerial.println(myGNSS.minfo.extension[i]);
    }
    DSerial.println();
    DSerial.println(F("Done!"));
}
